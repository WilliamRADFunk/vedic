...
public class Vid_Function : Vid_Expression
{
    ...
    public Vid_Function(int numInputs) : base()
    {
        inputs = new Vid_ObjectInputs(numInputs);
        prefix_head = Vid_Prefix.Vid_Common_Prefix.PUBLIC;
        prefix_returnVal = Vid_Prefix.Vid_returnValue.VOID;
        parameters_names = new List<String>();
        prefix = new Dictionary<Vid_Prefix, string>();
        functionName = "defaultName" + id;
        id++;
    }
    ...
    public override void stringify()
    {
        ...
        mainStringBuilder.Append(text_outPut);
        if (sequence != null)
        {
            sequence.stringify();
        }
    ...
    {
        if (data.getVidData_type() != VidData_Type.DATABASE
                | data.getVidData_type() != VidData_Type.FUNCTION
    ...
    public void toggleVid_Prefix_return()
    {
        switch (prefix_returnVal)
        {
            ...
            case Vid_Prefix.Vid_returnValue.CLASS:
                prefix_head = Vid_Prefix.Vid_Common_Prefix.PUBLIC;
                break;
            case Vid_Prefix.Vid_returnValue.NUMBER:
                prefix_head = Vid_Prefix.Vid_Common_Prefix.PUBLIC;
                break;
            default:
                prefix_head = Vid_Prefix.Vid_Common_Prefix.PUBLIC;
